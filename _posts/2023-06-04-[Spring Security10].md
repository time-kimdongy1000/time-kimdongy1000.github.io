...
.permitAl().anyRequest().authenticated();
...
```
로그인 페이지를 좀더 이쁘게 만들기 위해서 다음과 같은 css 를 추가하겠습니다 그러면 이와 같은 화면이 나오게 되는데 사실 부트스트랩이 제공하는 css 그냥 가져온것이니 
크게 뭐가 없다 나도 디자인이나 이런쪽에서는 완전 무뇌쪽이다 
